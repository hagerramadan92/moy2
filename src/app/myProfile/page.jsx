...
export default function MyProfilePage() {
  ...
  const handleDeleteAccount = () => {
    ...
    if (!userData || !userData.phoneNumber) {
      Swal.fire({
        ...
        customClass: {
          ...
          confirmButton:
            "rounded-xl font-bold px-4 sm:px-6 py-2 text-sm sm:text-base",
          title: "text-sm",
    ...
    Swal.fire({
      ...
      customClass: {
        popup: "rounded-2xl border border-border shadow-xl mx-4",
        confirmButton:
          "rounded-xl font-bold px-4 sm:px-6 py-2 ml-2 text-sm sm:text-base",
        cancelButton:
          "rounded-xl font-bold px-4 sm:px-6 py-2 text-sm sm:text-base",
        ...
        fetch(deleteUrl, {
          ...
          .then(async (response) => {
            ...
            if (response.ok) {
              ...
              throw new Error(
                data.message || data.error || "حدث خطأ أثناء حذف الحساب",
              );
            }
          })
          .catch((error) => {
      ...
      try {
        ...
        if (response.ok && data.status) {
          ...
          if (data.data) {
            ...
            setPhone(
              data.data.phone_number || data.data.phone || phoneFromOtp || "",
            );
  ...
  const handleDeleteAddress = async (addressId, addressName, event) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://dashboard.waytmiah.com/api/v1/addresses/${addressId}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        const errorMessage =
          data.message ||
          data.error ||
          "فشل حذف العنوان. يرجى المحاولة مرة أخرى";
  ...
  const handleToggleFavorite = async (
    ...
    const loadingToast = toast.loading(
      currentFavoriteStatus
        ? "جاري الإزالة من المفضلة..."
        : "جاري الإضافة إلى المفضلة...",
      {
        style: {
          background: "var(--background)",
          border: "1px solid var(--border)",
    ...
    try {
      ...
      if (updateResponse.ok && updateResult.status) {
        ...
        // تحديث قائمة addresses
        setAddresses((prevAddresses) =>
          prevAddresses.map((addr) =>
            addr.id === addressId
              ? { ...addr, is_favorite: newFavoriteStatus }
              : addr,
          ),
        );
        ...
        setPlaces((prevPlaces) =>
          prevPlaces.map((place) => {
            if (typeof place === "object" && place.id === addressId) {
              return { ...place, is_favorite: newFavoriteStatus };
            }
            return place;
          }),
        );
        ...
        if (newFavoriteStatus) {
          ...
          if (updatedAddress) {
            setFavorites((prev) => [
              ...prev,
              { ...updatedAddress, is_favorite: true },
            ]);
          }
        } else {
          // إذا تمت الإزالة من المفضلة
          setFavorites((prev) =>
            prev.filter((fav) => {
              if (typeof fav === "object" && fav.id) {
                return fav.id !== addressId;
        ...
        // تحديث العنوان المحدد إذا كان مفتوحاً
        if (selectedAddress?.id === addressId) {
          setSelectedAddress((prev) => ({
            ...prev,
            is_favorite: newFavoriteStatus,
          }));
          // تحديث نموذج التعديل إذا كان مفتوحاً
          if (isEditingAddress) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground">
                هل أنت متأكد من حذف "{favoriteName}"؟
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-3 p-2">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground">
                سيتم حذف الصورة عند حفظ التغييرات. هل تريد المتابعة؟
              </p>
            </div>
          </div>
          <div className="flex gap-2 justify-end">
            ...
            <button
              onClick={() => {
                ...
                setAvatarPreview(null);
                toast.success(
                  "سيتم حذف الصورة عند حفظ التغييرات. يرجى الضغط على حفظ التغييرات",
                  {
          ...
          try {
            ...
            if (removeImage) {
              ...
              const avatarFromResponse =
                data.data?.user?.avatar || data.data?.avatar;
  ...
  return (
    <div className="flex flex-col gap-4 sm:gap-6 md:gap-8 fade-in-up px-2 sm:px-0">
      ...
      <div className="bg-white dark:bg-card rounded-xl sm:rounded-xl shadow-xl overflow-hidden border border-border/60">
        ...
        <div className="p-4 sm:p-6 md:p-8 lg:p-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            {/* Full Name */}
            <div className="space-y-1 sm:space-y-2">
              <label className="block text-xs sm:text-sm font-bold text-foreground/80 mr-2">
                الاسم الكامل
              </label>
              <div className="relative group">
                <div
                  className={`absolute top-1/2 -translate-y-1/2 right-3 sm:right-4 transition-colors ${errors.fullName ? "text-destructive" : "text-muted-foreground/60 group-focus-within:text-primary"}`}
                >
      ...
      <div className="bg-white dark:bg-card rounded-xl sm:rounded-xl md:rounded-xl overflow-hidden shadow-xl border border-border/60">
        <div className="p-4 sm:p-6 md:p-8 lg:p-10">
          <h3 className="text-lg sm:text-xl md:text-2xl font-black mb-4 sm:mb-6 md:mb-8 text-foreground">
            إعدادات الحساب
          </h3>
          ...
          <div className="flex flex-col gap-3 sm:gap-4">
            ...
            >
              <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
                <div
                  className={`p-2.5 sm:p-3 md:p-3.5 rounded-xl sm:rounded-xl shadow-lg group-hover:shadow-xl group-hover:scale-110 transition-all ${
                    notificationsActive
                ...
                >
                  <FaBell
                    className={`w-4 h-4 sm:w-5 sm:h-5 ${notificationsActive ? "text-white" : "text-muted-foreground"}`}
                  />
                </div>
                <div>
                  <h4 className="text-sm sm:text-base md:text-lg font-bold mb-0.5 sm:mb-1 text-foreground">
                    تفعيل الإشعارات
                  </h4>
                  <p className="text-xs sm:text-sm text-muted-foreground">
                    استقبال التنبيهات والتحديثات
                  </p>
                </div>
              </div>
              ...
              >
                <div
                  className={`absolute top-0.5 bg-white w-5 h-5 sm:w-6 sm:h-6 rounded-full shadow-md transition-transform duration-300 ${
            ...
            >
              <div className="flex items-center gap-2 sm:gap-3 md:gap-4 flex-1 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  <h4 className="text-sm sm:text-base md:text-lg font-bold mb-0.5 sm:mb-1 text-foreground truncate">
                    الأماكن المحفوظة
                  </h4>
                  <p className="text-xs sm:text-sm text-muted-foreground">
                    احفظ مواقعك المفضلة
              ...
              <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                <FaChevronLeft
                  className={`text-muted-foreground group-hover:text-primary transition-transform duration-300 text-lg sm:text-xl ${showSavedPlaces ? "rotate-[-90deg]" : ""}`}
                />
              </div>
            </div>
            {showSavedPlaces && (
                        ...
                        return typeof place === "object" && place.id ? (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                ...
                                className={`transition-all p-2 rounded-lg hover:scale-110 ${
                                  place.is_favorite
                                    ...
                                    : "text-muted-foreground hover:text-[#579BE8] hover:bg-[#579BE8]/10"
                                }`}
                                title={
                                  place.is_favorite
                                    ? "إزالة من المفضلة"
                              ...
                              >
                                <FaStar
                                  className={`w-4 h-4 ${place.is_favorite ? "fill-current" : ""}`}
                                />
                              </button>
                              ...
                              <button
                                ...
                                className="transition-opacity p-2 hover:bg-red-100 dark:hover:bg-red-900/20 rounded-lg text-red-600 dark:text-red-400 hover:scale-110"
                                title="حذف العنوان"
                              >
                          ...
                          <div
                            key={uniqueKey}
                            className="flex items-start justify-between gap-2 sm:gap-3 md:gap-4 p-3 sm:p-4 bg-secondary/30 border border-border/50 rounded-xl sm:rounded-xl fade-in-up hover:bg-secondary/40 hover:border-primary/30 transition-all"
                          >
            ...
            >
              <div className="flex items-center gap-2 sm:gap-3 md:gap-4 flex-1 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs sm:text-sm text-muted-foreground">
                    احتفظ بأماكنك المفضلة
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                <FaChevronLeft
                  className={`text-muted-foreground group-hover:text-primary transition-transform duration-300 text-lg sm:text-xl ${showFavorites ? "rotate-[-90deg]" : ""}`}
                />
              </div>
            </div>
                      ...
                      .map((fav, i) => {
                        ...
                        return typeof fav === "object" && fav.id ? (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              {/* زر التبديل للمفضلة - هنا سيكون دائماً true */}
                              <button
                                onClick={(e) =>
                                  handleToggleFavorite(
                                    fav.id,
                                ...
                                className="transition-all p-2 rounded-lg hover:scale-110 text-[#579BE8] hover:bg-[#579BE8]/10"
      ...
      <div className="bg-white dark:bg-card rounded-xl sm:rounded-xl md:rounded-xl overflow-hidden shadow-xl border border-border/60">
        <div className="p-4 sm:p-6 md:p-8 lg:p-10">
          ...
          {/* Location Sharing */}
          <div
            className={`flex items-center justify-between p-3 sm:p-4 md:p-5 rounded-xl sm:rounded-xl border transition-all cursor-pointer group mb-3 sm:mb-4 ${
          ...
          >
            <div className="flex items-center gap-2 sm:gap-3 md:gap-4 flex-1 min-w-0">
              <div
                className={`p-2 sm:p-2.5 md:p-3.5 rounded-xl sm:rounded-xl shadow-lg group-hover:shadow-xl group-hover:scale-110 transition-all flex-shrink-0 ${
                  locationSharingActive
                    ? "bg-gradient-to-br from-[#579BE8] to-[#315782]"
                    : "bg-muted-foreground/30"
                }`}
              >
                <BiCurrentLocation
                  className={`w-4 h-4 sm:w-5 sm:h-5 ${locationSharingActive ? "text-white" : "text-muted-foreground"}`}
                />
              </div>
          ...
          <div className="flex items-center justify-between p-3 sm:p-4 md:p-5 rounded-xl sm:rounded-xl border border-border/50 bg-secondary/5 hover:bg-secondary/10 hover:border-primary/30 transition-all cursor-pointer group mb-3 sm:mb-4">
            ...
            <Link
              href="/privacy"
              className="text-[#579BE8] font-bold text-xs sm:text-sm hover:text-[#315782] transition-colors flex items-center gap-1 sm:gap-2"
            >
              عرض التفاصيل
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div className="bg-secondary/30 rounded-xl p-5 border border-border/50 flex items-center justify-center">
                        <div className="flex items-center gap-3">
                          ...
                          >
                            <FaStar
                              className={`w-5 h-5 ${editAddressForm.is_favorite ? "text-[#579BE8]" : "text-muted-foreground"}`}
                            />
                            إضافة إلى المفضلة
                          </label>
                        </div>
