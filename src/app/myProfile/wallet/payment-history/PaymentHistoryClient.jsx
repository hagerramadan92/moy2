  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-white dark:bg-card rounded-3xl shadow-2xl max-w-3xl w-full max-h-full "
        onClick={(e) => e.stopPropagation()}
      >
        {/* الهيدر */}
        <div className="bg-gradient-to-l from-[#579BE8] to-[#315782] p-2 md:p-6 text-white sticky top-0 rounded-t-2xl">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-1 md:gap-4">
              <div className=" p-1 md:p-3 bg-white/20 backdrop-blur-sm rounded-lg md:rounded-2xl">
                {type === "deposit" ? (
                  <FaArrowDown size={20} />
                ) : (
                  <FaArrowUp size={20} />
          ...
          <div className="flex items-center gap-3 mt-4">
            <span
              className={` md:px-3 px-1 py-1.5 rounded-xl text-xs flex items-center font-bold ${getStatusColor(transaction.status)}`}
            >
              <span
                className={`inline-block w-2 h-2 rounded-full ${getStatusColorClass(transaction.status)} ml-1.5`}
              ></span>
              <span className="text-[10px]">{getStatusText(transaction.status)}</span>
            </span>
            <span className="px-3 py-1.5 bg-white/20 backdrop-blur-sm rounded-xl text-xs">
        ...
        <div className="p-6">
          ...
          <div className="text-center mb-6">
            ...
            <div className="flex items-center justify-center gap-2">
              <span
                className={`text-xl md:text-4xl font-black ${type === "deposit" ? "text-green-600" : "text-gray-900 dark:text-white"}`}
              >
                {type === "deposit" ? "+" : "-"}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-xl">
              ...
              <p className="font-mono text-sm font-bold">{transaction.id}</p>
            </div>
            ...
            {/* {transaction.processed_at && (
                            <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-xl">
                                <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">تاريخ المعالجة</p>
                                <p className="font-bold text-sm">{formatDate(transaction.processed_at)}</p>
                            </div>
                        )} */}
            ...
            {/* {transaction.payment_transaction_id && (
                            <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-xl md:col-span-2">
                                <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">معرف معاملة بوابة الدفع</p>
                                <p className="font-mono text-sm font-bold">{transaction.payment_transaction_id}</p>
                            </div>
                        )} */}
          ...
          <div className="flex gap-3 mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
            <button
              onClick={() => {
                ...
                Swal.fire({
                  title: "تم النسخ",
                  text: "تم نسخ معرف المعاملة",
                  icon: "success",
                  timer: 1500,
                  showConfirmButton: false,
                });
              }}
              className="flex-1 py-3 px-4 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-xl font-bold text-sm transition-colors"
            >
              نسخ المعرف
            </button>
            <button
              onClick={onClose}
              className="flex-1 py-3 px-4 bg-[#579BE8] text-white hover:bg-[#4a8ad0] rounded-xl font-bold text-sm transition-colors"
            >
              إغلاق
            </button>
          </div>
...
export default function PaymentHistoryClient() {
  ...
  const updateURL = useCallback(() => {
    ...
    const newUrl = queryString
      ? `/myProfile/wallet/payment-history?${queryString}`
      : "/myProfile/wallet/payment-history";
  ...
  const getTransactionTitle = (transaction) => {
    ...
    if (type.includes("deposit_pending") && transaction.status === "pending")
      return "إيداع معلق - قيد التأكيد";
    if (type.includes("deposit_pending") && transaction.status === "completed")
      return "إيداع (مكتمل)";
    ...
    <div className="space-y-4 sm:space-y-6 md:space-y-8 fade-in-up p-2 sm:p-4 md:p-6 w-full">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4 md:gap-6 w-full">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="w-full bg-gradient-to-br from-[#579BE8] via-[#579BE8] to-[#315782] text-white rounded-xl sm:rounded-2xl md:rounded-3xl p-3 sm:p-4 md:p-6 shadow-xl relative overflow-hidden"
          >
        ...
        <div className="mt-1 sm:mt-3 md:mt-5 pt-0 bg-white dark:bg-card rounded-xl sm:rounded-2xl md:rounded-3xl p-2 sm:p-3 md:p-6 shadow-sm border border-border/60 w-full">
          ...
          <div className="flex flex-wrap gap-1.5 sm:gap-2 w-full">
            {[1, 2, 3, 4, 5].map((i) => (
              <div
                key={i}
                className="h-8 sm:h-9 md:h-10 w-16 sm:w-20 md:w-24 bg-gray-200 dark:bg-gray-700 rounded-lg sm:rounded-xl animate-pulse"
              ></div>
            ))}
          </div>
        </div>
        ...
        <div className="p-3 sm:p-4 md:p-6 border-b border-border/50 flex flex-col sm:flex-row gap-2 sm:gap-3 md:gap-4 items-start sm:items-center justify-between w-full">
          ...
          <div className="flex bg-secondary/30 p-0.5 sm:p-1 rounded-xl sm:rounded-2xl">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-8 sm:h-9 md:h-10 w-14 sm:w-16 md:w-20 bg-gray-200 dark:bg-gray-700 rounded-lg sm:rounded-xl animate-pulse ml-0.5 sm:ml-1"
              ></div>
            ))}
          </div>
        </div>
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <div className="flex items-center gap-0.5 sm:gap-1">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-7 w-7 sm:h-8 sm:w-8 md:h-9 md:w-9 bg-gray-200 dark:bg-gray-700 rounded-lg sm:rounded-xl animate-pulse"
                ></div>
              ))}
            </div>
            <div className="h-7 w-7 sm:h-8 sm:w-8 md:h-9 md:w-9 bg-gray-200 dark:bg-gray-700 rounded-lg sm:rounded-xl animate-pulse"></div>
        ...
        <div className="bg-gradient-to-br from-[#579BE8] via-[#579BE8] to-[#315782] text-white rounded-xl md:rounded-3xl p-3 md:p-6 shadow-xl relative overflow-hidden group hover:shadow-2xl hover:-translate-y-1 transition-all w-full">
          ...
          <div className="relative z-10">
            ...
            <div className="flex items-center gap-2">
              <h3 className="text-xl md:text-4xl font-black drop-shadow-lg">
      ...
      <div className="w-full">
        <div className="md:gap-6 grid grid-cols-2 md:grid-cols-3 gap-3 w-full mb-6">
          ...
          <div className="space-y-0 w-full max-w-md">
            <div className="relative w-full">
              <IoSearchOutline
                className="absolute right-4 top-1/2 -translate-y-1/2 text-[#579BE8]"
                size={20}
              />
              <input
                type="text"
              ...
              <SelectContent className="text-right max-h-[300px]">
                {statusOptions.map((status) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span
                        className={`w-2 h-2 rounded-full ${getStatusColorClass(status.id)}`}
                      ></span>
        ...
        <div className="mt-1 md:mt-5 pt-0 bg-white dark:bg-card rounded-3xl p-3 md:p-6 shadow-sm border border-border/60 w-full">
          <h4 className="text-sm md:text-base font-medium text-gray-700 dark:text-gray-300 mb-3">
            فلاتر سريعة
          </h4>
          <div className="flex flex-wrap gap-2 w-full">
            {quickFilters.map((filter) => (
              <button
                ...
                className={`px-4 py-2 rounded-xl border-2 text-muted-foreground transition-all text-sm font-medium ${
                    ...
                    {transactions.map((transaction) => (
                      ...
                      >
                        <td className="px-6 py-5">
                          <div >
                            <span className="font-bold text-[#579BE8] hover:underline text-xs md:text-sm whitespace-nowrap  max-w-[20px] truncate">
                             <span className="font-bold text-[#579BE8] hover:underline text-xs md:text-sm whitespace-nowrap max-w-[20px] truncate">
  {(transaction.reference || transaction.id).slice(0, 10)}
                        ...
                        <td className="px-6 py-5 text-center">
                          <div
                            className={`flex items-center justify-center gap-1 font-black text-lg ${getTransactionType(transaction) === "deposit" ? "text-green-600" : "text-foreground"}`}
                          >
                            {getTransactionType(transaction) === "deposit"
                              ? "+"
                              : "-"}
    ...
    */}
                        ...
                        <td className="px-6 py-5 text-center">
                          <span
                            className={`inline-flex items-center gap-1.5  md:px-3 px-1 py-1.5 rounded-xl text-xs font-bold ${getStatusColor(transaction.status)}`}
                          >
                            <span
                              className={`w-2 h-2 rounded-full ${getStatusColorClass(transaction.status)}`}
                            ></span>
                           <span className="whitespace-nowrap"> {getStatusText(transaction.status)}</span>
                          </span>
                        </td>
                      </motion.tr>
                    ))}
                  ...
                  {transactions.length === 0 && (
                    <tr>
                      <td
                        colSpan="6"
                        className="px-6 py-10 text-center text-muted-foreground w-full"
                      >
