...
export async function POST(req, { params }) {
  try {
    ...
    if (!accessToken) {
      return NextResponse.json(
        { 
          message: 'رمز المصادقة مطلوب لتجديد العقد',
    ...
    const response = await fetch(`https://dashboard.waytmiah.com/api/v1/contracts/${id}/renew`, {
      method: 'POST',
      headers: headers,
    });
    ...
    if (response.ok) {
      ...
      return NextResponse.json(
        { 
          message: data.message || data.error || 'فشل إرسال طلب تجديد العقد',
          errors: data.errors 
        },
    ...
    console.error('Renew Contract API error:', error);
    return NextResponse.json(
      { message: 'حدث خطأ أثناء إرسال طلب تجديد العقد' },
      { status: 500 }
    );
  }
}
